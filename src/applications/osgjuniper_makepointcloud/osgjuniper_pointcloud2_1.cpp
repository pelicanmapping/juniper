...
class SimplePointSource : public PointSource
{
public:
    
    class SimplePointCursor : public PointCursor
    {
    public:
        
        SimplePointCursor(const std::string& filename):
          _filename(filename)
        {
        ...
        virtual bool nextPoint(Point& point)
        {
            ...
            {
                osg::Vec3 vert;
                _in.read((char*)(&vert[0]), sizeof(float) * 3);
    ...
    SimplePointSource(const std::string &filename):
      _filename(filename)
    {
    }
    ...
    virtual PointCursor* createPointCursor()
    {
        return new SimplePointCursor( _filename );
    }
protected:
    std::string _filename;
};
    ...
    public:
        ...
        virtual bool nextPoint(Point& point)
        {
            if (!_in.eof())
            {
                ...
                osg::Vec4 col;
                float size; 
                _in.read((char*)(&vert[0]), 12);
                _in.read((char*)(&norm[0]), 12);
                _in.read((char*)(&col[0]), 12);
                _in.read((char*)(&size), 4);
                col *= 1.0/255.0;
                col[3] = 1.0;
...
void SPTNode::setPoints(PointList &points)
{
    ...
    {     
        ...
        if (rangeIndex.find(points.front()._level) == rangeIndex.end())
        {
            ...
            osg::notify(osg::NOTICE) << "Initialized range for " << points.front()._level << std::endl;
        }
...
class MakePagedSceneVisitor : public OctreeNodeVisitor
{
public:
    MakePagedSceneVisitor()
    {
    }
    ...
    {        
        ...
        pagedLOD->setRadius(range);
        pagedLOD->setCenter(center);

        osg::ref_ptr< osg::Geode > geode = new osg::Geode;
        osg::ref_ptr< osg::Box > box = new osg::Box(node.getBoundingBox().center(),
                                                    node.getBoundingBox().xMax() - node.getBoundingBox().xMin(),
                                                    node.getBoundingBox().yMax() - node.getBoundingBox().yMin(),
                                                    node.getBoundingBox().zMax() - node.getBoundingBox().zMin());        
        osg::ref_ptr<osg::ShapeDrawable> shapeDrawable = new osg::ShapeDrawable(box);
        shapeDrawable->setColor(Utils::randomColor());
        ...
        if (node.isSplit())
        {
            ...
            {
                std::string uri = createURI(*node.getChildren()[i].get());
                pagedLOD->setFileName(i+1, uri);
                pagedLOD->setRange(i+1, 0, node.getChildren()[i]->getBoundingBox().radius() * RADIUS_FACTOR);
            }
        }
...
public:
      ...
      _numPoints(0)
    {
      ...
      void addPoint(const Point& point)
      {
          ...
          float size = 1.0f;
          _out.write((char*)&point._position._v,12);
          _out.write((char*)&point._normal._v, 12);
          _out.write((char*)&color._v, 12);
          _out.write((char*)&size, 4);
      }
...
{
public:
    MakeSceneVisitor(const std::string &filename, const std::string& driver, unsigned int innerMaxLevel, unsigned int outerMaxLevel):
      _filename(filename),
      _driver(driver),
    ...
    virtual void apply(OctreeNode& node)
    {
        ...
        osg::ref_ptr< PointSource > pointSource = PointSource::loadPointSource( _filename );

        osg::ref_ptr< PointCursor > pointCursor = pointSource->createPointCursor();
        ...
        unsigned int numPointsRead     = 0;
        unsigned int numPointsAdded    = 0;
        unsigned int numPointsRejected = 0;
        ...
        for (unsigned int i = 0; i < 8; ++i)
        {
            if (rejectionFiles[i]->getNeedsProcessed())
            {
                ...
                MakeSceneVisitor msv(rejectionFiles[i]->getFilename(), "spawar", _innerMaxLevel, _outerMaxLevel);
...
osg::Node* makeSceneFromPointSource(PointSource *source)
{
    ...
    /*MakePagedSceneVisitor pagedVisitor;
    root->accept(pagedVisitor);*/
...
osg::Node* makeScene()
{
    ...
    root->setBoundingBox(osg::BoundingBox(osg::Vec3(-1,-1,-1), osg::Vec3(1,1,1)));    
...
void makeSimple(const std::string& input, const std::string& output)
{
    ...
    osg::ref_ptr< PointSource > inputSource = new CSVPointSource(input);
...
void makeSimpleFromDirectory(const std::string& directory)
{
        ...
        {
            std::string inputFilename  = filenames[i];
            OSG_NOTICE << "Processing file " << i+1 << " of " << filenames.size() << ":" << inputFilename  << std::endl;
            std::string outputFilename = osgDB::getNameLessExtension(inputFilename) + ".pts";
            OSG_NOTICE << "Converting " << inputFilename << " to " << outputFilename << std::endl;
            makeSimple(inputFilename, outputFilename);
        }*/
}
...
int main(int argc, char** argv)
{
    ...
    //makeSimpleFromDirectory("F:\\geodata\\paul_levy_2005_PostKatrina_35192");
    //return 0;
    
    osg::Node* root = NULL;
    osg::ref_ptr< osg::Node > loadedModel = osgDB::readNodeFiles( arguments );

    //makeSimple("F:\\geodata\\paul_levy_2005_PostKatrina_35192\\30089_15_63_raw.txt", "F:\\geodata\\paul_levy_2005_PostKatrina_35192\\30089_15_63_raw.pts");
    //return 0;
    ...
    OSG_NOTICE << "Bounding box is " << bb._min << " to " << bb._max << std::endl;
    osg::ref_ptr< OctreeNode > sceneRoot = new OctreeNode();
    sceneRoot->setBoundingBox( bb );
    MakeSceneVisitor msv(inputFilename, "csv", 7, 11);
