...
*/
#include <osgJuniper/DirectoryPointTileStore>
...
DirectoryPointTileStore::DirectoryPointTileStore(const std::string& path):
	_path(path)
{
}

const std::string& DirectoryPointTileStore::getPath() const
{
	return _path;
}

std::string DirectoryPointTileStore::getKeyPath(const OctreeId& id)
{
	//static long guid = 0;
...
bool DirectoryPointTileStore::get(const OctreeId& id, PointList& points)
{	
...
void DirectoryPointTileStore::set(const OctreeId& id, PointList& points, bool append)
{
	if (!append)	
	{
		remove(id);
...
void DirectoryPointTileStore::remove(const OctreeId& id)
{
	std::string keyPath = getKeyPath(id);
