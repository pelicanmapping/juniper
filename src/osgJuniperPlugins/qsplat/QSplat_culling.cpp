                                      ...
                                      float cx, float cy, float cz, float r)
{
    ...
    for (int i=0; i < numnodes; i++, here += _nodeSize, there += _nodeSize*numchildren + grandchildren) {
        ...
        if (_overallSplatSize) {
            splatsize = myr/_radius * MINSIZE_MAX;
        }
...
public:
    ...
    void select() {
        ...
        if (!_previousTime) {
            ...
            _previousTime = now;
            _minsize = refine();
            _builder->setupBuild(_viewerInfo, _minsize, true, false);
            ...
            //if (dt > _checkRate) {
                ...
                if (_isOnScreen) {
                    ...
                    if (!_askRun) {
                        ...
                        if (canRefine()) minsize = refine();
                        _builder->setupBuild(_viewerInfo, minsize, true, false);
                        _operationThread->add(_builder.get());
                        _askRun = true;
