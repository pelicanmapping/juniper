...
inline unsigned int SWAP_LONG(unsigned int x)
{
    return ( ((x) << 24) | (((x) << 8) & 0x00ff0000) | (((x) >> 8) & 0x0000ff00) | ((x) >> 24) );
}

inline unsigned short SWAP_SHORT(unsigned short x)
{
    return ( (((x) & 0xff) << 8) | ((unsigned short)(x) >> 8) );
}

#ifdef WE_ARE_LITTLE_ENDIAN
inline void FIX_SHORT(unsigned short& x) 
{
    unsigned short * pointer = (unsigned short *)(&x);
    *pointer = SWAP_SHORT(*pointer);
}
...
void FIX_LONG(T& x)
{
    ...
    *pointer = SWAP_LONG(*pointer);
}

inline float FIX_FLOAT(float f)
{
    unsigned int* source = (unsigned int *)(&f);
    unsigned int dest;
    dest = SWAP_LONG( (*source) );
